...
df = pd.read_csv(r"C:\Users\amrit\OneDrive\Desktop\blackhouse\SOUN_2025-05-02 00_00_00+00_00.csv")
...
sns.scatterplot(x='signed_volume', y='price_change', data=resampled, alpha=0.5)
plt.plot(resampled['signed_volume'], model.predict(X), color='red', label='Regression Line')
plt.legend()
import numpy as np
resampled['log_volume'] = np.log1p(np.abs(resampled['signed_volume'])) * np.sign(resampled['signed_volume'])
resampled['log_price_change'] = np.log1p(np.abs(resampled['price_change'])) * np.sign(resampled['price_change'])

# Fit log-log model
X_log = resampled['log_volume'].values.reshape(-1, 1)
y_log = resampled['log_price_change'].values
log_model = LinearRegression().fit(X_log, y_log)

print("Log-Log Impact Coefficient:", log_model.coef_[0])
